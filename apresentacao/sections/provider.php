...
$query = "SELECT * FROM sgc_".$user['customer_code']."_providers WHERE id = :id";
...
$ret = sgc_save_db('sgc_'.$user['customer_code'].'_providers', $data, $id);
...
$data_timeline['title'] = 'Atualizou este fornecedor';
$data_timeline['content'] = '<p style="color: #3c8dbc"><strong>Tipo</strong></p> <strong>De</strong> '.$up_from.' <br><strong>para</strong> '.$up_to;
$new_timeline = sgc_save_db('sgc_'.$user['customer_code'].'_providers_updates', $data_timeline, 0);
}
...
$query = "SELECT a.*,u.name AS name_created,um.name AS name_modified FROM sgc_".$user['customer_code']."_providers AS a LEFT JOIN users AS u ON u.id = a.created_user LEFT JOIN users AS um ON um.id = a.modified_user WHERE a.id = :id";
$search = conecta()->prepare($query);
...
//timeline
$query = "SELECT a.*,u.name AS name_user FROM sgc_".$user['customer_code']."_providers_updates AS a LEFT JOIN users AS u ON u.id = a.created_user WHERE a.item_id = :id ORDER BY a.id DESC";
$search = conecta()->prepare($query);
$search->bindValue(':id', $_POST['id']);
$search->execute();
...
//Popula Categorias
$query = "SELECT id,name FROM sgc_".$user['customer_code']."_providers_categories WHERE status = 1 ORDER BY name";
$search = conecta()->prepare($query);
$search->execute();
...
<section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="javascript:void();" onclick="open_target('target=providers');">Gestão de fornecedores</a></li>
...
<div class="tab-pane active" id="cadastro">
    ...
    <div class="box-body">
      ...
      <div class="row">
        <div class="col-sm-6">
          <label>
              <input type="radio" name="type" onclick="type_provider($(this));" id="type1" value="1" <?php echo ($row['type'] == 1)?'checked':'';?>>
               Pessoa física
          </label>
        </div>
        <div class="col-sm-6">
          <label>
              <input type="radio" name="type" onclick="type_provider($(this));" id="type2" value="2" <?php echo ($row['type'] == 2)?'checked':'';?>>
               Pessoa jurídica
          </label>
        </div>
      </div>
...
<div class="row">
  ...
  <div class="col-sm-6">
    <button type="button" onclick="open_target('target=providers', $(this));" class="btn btn-default pull-right rr"> Voltar</button>
  </div>
...
<h3 class="timeline-header"><span style="color: #3c8dbc";>Sistema</span> Este fornecedor não possui atualizações</h3>
...
var params 			= 'target=provider&exec=save_provider&'+form.serialize();
