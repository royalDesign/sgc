...
$query = "SELECT * FROM sgc_".$user['customer_code']."_products_categories WHERE id = :id";
...
$ret = sgc_save_db('sgc_'.$user['customer_code'].'_products_categories', $data, $id);
...
$new_timeline = sgc_save_db('sgc_'.$user['customer_code'].'_products_categories_updates', $data_timeline, 0);
}
...
$query = "SELECT a.*,u.name AS name_created,um.name AS name_modified FROM sgc_".$user['customer_code']."_products_categories AS a LEFT JOIN users AS u ON u.id = a.created_user LEFT JOIN users AS um ON um.id = a.modified_user WHERE a.id = :id";
$search = conecta()->prepare($query);
...
//timeline
$query = "SELECT a.*,u.name AS name_user FROM sgc_".$user['customer_code']."_products_categories_updates AS a LEFT JOIN users AS u ON u.id = a.created_user WHERE a.item_id = :id ORDER BY a.id DESC";
$search = conecta()->prepare($query);
$search->bindValue(':id', $_POST['id']);
$search->execute();
